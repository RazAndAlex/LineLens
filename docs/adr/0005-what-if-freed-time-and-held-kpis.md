...
because its numerator and denominator accumulate in a different operation order —
well inside the tolerance. Meanwhile Availability rises (more run time, less
unplanned — and the denominator `run + unplanned + idle` is invariant, since
freed time moves from `unplanned` to `run`), OEE rises with Availability, `good`
