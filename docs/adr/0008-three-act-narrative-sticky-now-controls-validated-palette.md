...
section; Charts → 6, Export → 7"). ADR-0005's *arithmetic* — freed time becomes
run time at the line's current effective speed, P/Q held, top-5 sliders (ADR-0005
Decisions 1–3) — is untouched; only *where on the page* the what-if renders
moves. The reversal is justified: with a forecast now in the page, the two
...
The narrative answers the reader's three questions in order — *what's wrong
with the data, where do we stand today, where is this heading* — and keeps the
